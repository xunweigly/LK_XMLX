...
using U8Login;
using UFIDA.U8.Framework.LTM.Service;
...
namespace U8SOFT.XMRZ
{
    class XunJia:IButtonEventHandler
    {
       ...
       //private const string SubId = "DP";		//需要根据各业务子系统进行替换
        ...
        public string Excute(VoucherProxy ReceiptObject, string PreExcuteResult)
        {
            ...
            Business dt3 = ReceiptObject.Businesses["LK1_0007_E003"];

            if (dt3.Rows.Count == 0)
            {
                MessageBox.Show("没有填写原料，无法提交！");
                return "<result><system result=\"false\" errinfo=\"" + "没有填写原料" + "\"/></result>";
    ...
    //        clsLogin nl = new clsLogin();
    //nl=(U8Login.clsLogin)ReceiptObject.NetLoginObject;
            //创建审批服务的客户端代理
